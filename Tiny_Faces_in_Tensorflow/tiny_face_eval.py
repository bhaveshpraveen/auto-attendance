...
def overlay_bounding_boxes(raw_img, refined_bboxes, lw):
  ...
  for r in refined_bboxes:
    ...
    cv2.imwrite(face_file_name, raw_img[_r[1]:_r[3], _r[0]:_r[2]])
    # raw_img = cv2.cvtColor(raw_img, cv2.COLOR_RGB2BGR)
...
def evaluate(weight_file_path=pickle_file_path, data_dir=data_dir, img=None, output_dir=None, prob_thresh=0.5, nms_thresh=0.1, lw=3, display=False):
 ...
 #    filenames.extend(glob.glob(os.path.join(data_dir, ext)))
  ...
  with tf.Session() as sess:
    ...
    # raw_img = cv2.cvtColor(raw_img, cv2.COLOR_RGB2BGR)
    # cv2.imwrite(os.path.join(output_dir, 'image_with_faces'), raw_img)
